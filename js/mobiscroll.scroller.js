(function ($, window, document, undefined) {
    ...
    function convert(w) {
        ...
        $.each(w, function (k, v, d, o) {
            ret.keys.push(k);
            ret.values.push(v);
            ret.attr.push(d);
            ret.orientation.push(o);
        });
        return ret;
    }
    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        function onMove(ev) {
            if (move) {
                if (isScrollable) {
                    ...
                    if($elm.attr('data-orientation') == 'vertical'){
                        if(Math.abs(stop2 - start2) > 3 || moved){
                            scroll(target, index, constrain(p + (start2 - stop2) / 140, min - 1, max + 1));
                            moved = true;
                        }
                    }else {
        ...
        function onEnd(ev) {
            ...
            if (move) {
                var time = new Date() - startTime,
                    ...
                    val2 = constrain(p + (start2 - stop2) / 140, min - 1, max + 1),
                    speed,
                    dist,
                    tindex,
                ...
                if (!moved) { // this is a "tap"
                    if($elm.attr('data-orientation') == 'vertical'){
                        var idx = Math.floor((stop2 - tleft) / 140),
                            li = $($('.dw-li', target)[idx]),
                            hl = isScrollable;
                    }else {
                            idx = Math.floor((stop - ttop) / itemHeight);
                            li = $($('.dw-li', target)[idx]);
                            hl = isScrollable;
                    }
        ...
        function generateWheelItems(i) {
            ...
            $.each(values, function (j, v) {
                html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight + 'px;line-height:' + itemHeight + 'px;">' + '<button class="btn-mobi"></button>' +
                    '<div class="dw-i"' + (lines > 1 ? ' style="line-height:' + Math.round(itemHeight / lines) + 'px;font-size:' + Math.round(itemHeight / lines * 0.8) + 'px;"' : '') + '>' + v + '<span class="sub-txt">' + attr[j] + '</span>' + '</div></div>';
                l++;
            });
        ...
        function scroll(t, index, val, time, active) {
            ...
            if (has3d) {
                if($elm.attr('data-orientation') == 'vertical'){
                    style[pr + 'Transform'] = 'translate3d(' + px + 'px' + ',0 , 0)';
                } else {
            ...
            if (trigger('validate', [$markup, index, time, dir]) !== false) {
                ...
                $('.dw-ul', $markup).each(function (i) {
                    ...
                    if (!(cell.hasClass('dw-sel')) || sc) {
                        ...
                        $('.dw-sel', t).prev().removeClass('prev');
                        $('.dw-sel', t).next().removeClass('next');
                        $('.dw-sel', t).removeClass('dw-sel');
                        cell.prev().addClass('prev');
                        cell.next().addClass('next');
